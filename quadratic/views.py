...
def quadratic_results(request):
    ...
    if a == 0:
        ifexeption = 1
        a_alert = u"коэффициент при первом слагаемом уравнения не может быть равным нулю"
    ...
    if ifexeption == 0:
        ...
        if discr < 0:
            discr_alert = u"Дискриминант меньше нуля, квадратное уравнение не имеет действительных решений."

        if discr == 0:
            x = (-b + discr ** (1/2.0)) / 2*a
            discr_alert = u"Дискриминант равен нулю, квадратное уравнение имеет один действительный корень: x1 = x2 = "
            x = (-b + discr ** (1/2.0)) / 2*a

        if discr > 0:
            x1 = (-b + discr ** (1/2.0)) / 2*a
            x2 = (-b - discr ** (1/2.0)) / 2*a
            discr_alert = u"Квадратное уравнение имеет два действительных корня: "
            x1_selfname = "x1 = "
            x2_selfname = ", x2 = "
